...
export const NewsServiceServiceName = "NewsService";
export class NewsServiceClientImpl implements NewsService {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || NewsServiceServiceName;
    this.rpc = rpc;
    this.getNews = this.getNews.bind(this);
  }
  getNews(request: Empty1): Promise<NewsList> {
    const data = Empty1.encode(request).finish();
    const promise = this.rpc.request(this.service, "GetNews", data);
    return promise.then((data) => NewsList.decode(_m0.Reader.create(data)));
  }
}
